...
def download_image(session, url):
    try:
        ...
        with open('./log2.txt', 'a') as f:
            f.write(f"{url} connection refused ** sleep 5 sec")
        print(f"{url} connection refused ** sleep 5 sec")
        import time
        time.sleep(5)
        return None
...
def download_faces(result_list, images_to_download, n_workers=2):
...
def run(args):
    ...
    # 중단된 이미지 부터 
    #     print(len(images_to_download), '/', len(images_to_download[34060:]))
    #     images_to_download = images_to_download[34060:]
        ...
        while n_processesed < n_data:
            if len(cropped_faces) < 1:
                ...
                if face is None:
                    ...
                    n_failed += 1
                    failed_list.append([title_id, image_name, idx, face])
                    continue
